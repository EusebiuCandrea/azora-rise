...
function marginBadge(price: number, cost: ProductCost | null) {
  ...
  const vatCollected = price * cost.vatRate
  const returnLoss = price * cost.returnRate
...
export function ProductList({ products }: ProductListProps) {
  ...
  return (
    <div className="bg-white border border-[#E7E5E4] rounded-xl shadow-sm overflow-hidden">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#1C1917] truncate max-w-[240px]">{product.title}</p>
      ...
      {/* Footer pagination */}
      <div className="px-4 py-3 border-t border-[#E7E5E4] bg-[#FAFAF9] flex items-center gap-1 text-sm text-[#78716C]">
        <span>←</span>
        <span className="px-2.5 py-1 bg-[#D4AF37] text-[#1C1917] rounded-lg text-xs font-semibold">1</span>
        <span className="px-2.5 py-1 hover:bg-[#F5F5F4] rounded-lg text-xs cursor-pointer">2</span>
        <span className="px-2.5 py-1 hover:bg-[#F5F5F4] rounded-lg text-xs cursor-pointer">3</span>
        <span>→</span>
        <span className="ml-auto text-xs text-[#78716C]">{products.length} produse în total</span>
      </div>
    </div>
  )
}
